...
1. **录制时游戏掉帧**：[`ReplayVideoCaptureSession`](../../Game/CombatReplay/Video/ReplayVideoCaptureSession.cs) 的 `OnReadbackComplete` 每帧 `new byte[width*height*4]`（2560×1440 ≈ 14MB），30fps 即 ~420MB/s 分配，GC 尖峰拖累主线程。
2. **没有声音**：游戏音频走 FMOD（`FMODUnity.RuntimeManager` / `FMOD.Studio`，见 [`AudioBankWarmer.cs`](../../Game/CombatReplay/Warmup/AudioBankWarmer.cs)），**无 Unity `AudioListener`/`AudioSource` 路径**，故 Unity `OnAudioFilterRead` 抓不到声音。
...
- mux 进程复用 [`FfmpegLocator`](../../Game/CombatReplay/Video/FfmpegLocator.cs) 的 ffmpeg。需校验该 ffmpeg 含 `aac` 编码器（原生 AAC 基本都有；缺则回退无声）。
...
- 音频 tap 与视频会话**并列**，挂在 [`CombatReplayVideoRecorder`](../../Game/CombatReplay/Video/CombatReplayVideoRecorder.cs) 的 `BeginRecording` / `OnPlaybackEnded` / `AbortActiveSession` 生命周期上。

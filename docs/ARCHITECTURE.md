...
The main plugin project targets `netstandard2.1`, uses C# 12, publicizes game assemblies, embeds data resources, and references the three unconditional child assemblies: `BazaarPlusPlus.ModApi`, `BazaarPlusPlus.Storage`, and `BazaarPlusPlus.Localization`. The voice-subtitle resource is downloaded from BazaarLine into `obj/` before resource generation on every build, then embedded with its stable manifest name; it is not stored in the working tree (`src/BazaarPlusPlus/BazaarPlusPlus.csproj:5-59`). The shared version is `BppVersion` in `Directory.Build.props` (`Directory.Build.props:3`).

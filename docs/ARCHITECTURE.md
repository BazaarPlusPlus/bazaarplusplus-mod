...
LiveBuildPanel reads live shop, board, and stash cards through `LiveCardSnapshotReader` (`src/BazaarPlusPlus/GameInterop/LiveCards/LiveCardSnapshotReader.cs:21-35`). Its recommendation backend is owned by `Game/LiveBuildPanel/Recommendations`: `BuildRecommendationRepository` loads the analyzer-v4 ten-win build corpus from an embedded seed, then a local cache, and refreshes that cache from the remote source before answering local recommendation queries (`src/BazaarPlusPlus/Game/LiveBuildPanel/Recommendations/BuildRecommendationRepository.cs:20-29`, `src/BazaarPlusPlus/Game/LiveBuildPanel/Recommendations/BuildRecommendationRepository.cs:52-89`). The embedded seed is supplied by the shared remote-data build pipeline rather than a checked-in JSON file (`src/BazaarPlusPlus/RemoteEmbeddedData.targets:27-31`). The panel exposes a manual final-build refresh action and recomputes recommendations from candidate state (`src/BazaarPlusPlus/Game/LiveBuildPanel/LiveBuildPanel.cs:217-247`, `src/BazaarPlusPlus/Game/LiveBuildPanel/LiveBuildPanel.cs:319-329`).
...
The settings dock registers all feature rows through `SettingsDockEntryRegistry` with order constants centralized in `BppSettingsDockOrder` (`src/BazaarPlusPlus/Game/Settings/BppSettingsDockOrder.cs`); the roster spans history, name override, legendary position, enchant preview, event and quest previews, combat status bar, Chinese locale, supporter list, voice subtitles, end-of-run screenshot, and BazaarDB upload (`src/BazaarPlusPlus/BppComposition.cs:118-130`). There is no BPP font selector because all BPP-owned game UI follows the game's font assets.
...
`PrepareOwnedText` preserves fail-before-create behavior for BPP TMP trees. `EnsureNativeTextCoverage` augments donor TMP without exposing how the chain is selected. `TryAttachPanel` resolves all requested roles (`BodyOnly` or `BodyAndHeading`) before atomically installing a panel-local `PanelTextSettings`; its scope is the only code that sets both `unityFont` and `unityFontDefinition`, checks external text against the actual selected font, and releases the panel settings. OS and emoji fallback remain disabled, global game settings are never mutated, and all Unity mutation is guarded to the initialization thread. The adapter restores donors, prunes destroyed donor bindings, invalidates outstanding scopes, and releases every owned handle/clone at teardown (`src/BazaarPlusPlus/GameInterop/Fonts/NativeGameTypography.cs`).

## Voice Subtitles

`Game/VoiceSubtitles` renders in-game voice-over subtitles (the BazaarLine integration): `VoiceSubtitlesModule` plus a `GameInterop/VoiceSubtitles` bridge module observe game VO playback (`src/BazaarPlusPlus/BppComposition.cs:106-116`), and `VoiceLineDisplayDispatcher`/`VersionLabelScanner` mountables render cues (`src/BazaarPlusPlus/BppComposition.cs:178-180`). The line catalog is supplied under its stable manifest name by the shared remote-data build pipeline, then supports background remote refresh at runtime (`src/BazaarPlusPlus/RemoteEmbeddedData.targets:21-25`, `src/BazaarPlusPlus/Game/VoiceSubtitles/VoiceLinesRepository.cs:16-20`). The feature is gated by a master toggle (default OFF) in `BazaarPlusPlus.cfg` `[VoiceSubtitles]` (`src/BazaarPlusPlus/Game/VoiceSubtitles/VoiceSubtitlesGate.cs:8-11`, `src/BazaarPlusPlus/Core/Config/BppConfig.cs:98-103`), with a four-state Subtitle Mode dock row registered via `VoiceSubtitlesSettingsDockEntry.RegisterAll` (`src/BazaarPlusPlus/BppComposition.cs:118-121`). Subtitle labels preserve the version-label donor font/material when it already covers Chinese. If it does not, mounting waits until the game's Chinese font configuration exists, then renders the Chinese row through `TextMeshProUGUI` using the game sans primary clone with its complete Chinese fallback chain and without synthetic bolding (`src/BazaarPlusPlus/Game/VoiceSubtitles/VersionLabelScanner.cs:43-69`, `src/BazaarPlusPlus/Game/VoiceSubtitles/VoiceLineDisplay.cs`).

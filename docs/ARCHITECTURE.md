...
BPP does not embed, extract, select, or create an operating-system font. `NativeGameFonts` retains the game's `zh-CN` TMP addressable references for the plugin session. Native TMP labels retain their donor primary font and material while a cloned primary receives the corresponding game fallback chain. BPP-created uGUI text uses `TextMeshProUGUI` with a clone of the game's loaded sans primary plus the complete ordered `zh-CN` Static → SecondSet → Dynamic chain; it must not bind the small Dynamic fallback atlas directly. UI Toolkit surfaces use the Dynamic asset's packaged `UnityEngine.Font` source because UI Toolkit cannot consume TMP assets directly (`src/BazaarPlusPlus/GameInterop/Fonts/NativeGameFonts.cs`, `src/BazaarPlusPlus/Game/CombatStatusBar/CombatStatusBar.Canvas.cs`, `src/BazaarPlusPlus/Game/CollectionPanel/Grid/CollectionSourceAttributionBadge.cs`).

...
- **As-built divergence from the spec**: the spec scoped this to AutoBazaar only and listed migrating the other features as a non-goal. In practice the abstraction generalized to `ComponentMount<T>` plus bespoke mount objects for Unity-owned workflows. Pure event consumers stay in `BppFeatureRegistry`; Run Logging moved there once its empty `MonoBehaviour` facade was removed. `HistoryPanelMount`, `CollectionPanelMount`, and `LiveBuildPanelMount` remain bespoke because they need dependency/lifecycle wiring beyond `ComponentMount<T>`. The current registrations live in `BppComposition.cs`.

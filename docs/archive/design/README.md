...
- [`2026-05-31-sell-hotkey-regression-debug-plan.md`](../../plans/sell-hotkey-regression-debug-plan.md) — 安装 BazaarPlusPlus 后官方出售物品快捷键偶发失效的生产调试方案：梳理 native SellItem 输入链路、tooltip/keybind/raycast 失效假设、诊断日志和修复验证矩阵（**Draft，未开工**）。
- [`2026-05-31-history-panel-hero-portrait-badge-design.md`](../../plans/history-panel-hero-portrait-badge.md) — HistoryPanel 英雄文本徽章（`VAN`/`PYG`）改真实头像 Sprite：走 `CollectionManager.GetDefaultHeroSkin` + `SkinAssetDataSO.LoadPortraitSpriteAsync`（异步 Addressables，无硬编码 key），在现有 `Label` 上设 `backgroundImage`，文本徽章降级为 fallback；含 token 防陈旧与逐文件源码锚点（**Draft，未开工**）。
...
- [`2026-06-09-collection-tag-chip-keyword-icon.md`](2026-06-09-collection-tag-chip-keyword-icon.md) — CollectionPanel TAG/关键词 chip 在文案前渲染游戏 keyword 配置的官方 TMP sprite 图标：`NativeTagDisplay.IconName` + `KeywordIconSpriteProvider` 抽出 UITK 可用 Sprite，chip 无图标时保持纯文字（**Implemented，游戏内视觉验证 pending**）。
- [`2026-06-08-package-merchant-portrait-plan.md`](2026-06-08-package-merchant-portrait-plan.md) — 给背包/Stash/手牌里的「包裹卡」（`EHiddenTag.Package`，名为 `<Merchant>'s Package`）叠加其对应商人的头像：映射走卡能力图里嵌入的商人 encounter GUID（`TPrerequisiteRun→TRunConditionalCurrentEncounter→TCardConditionalId`，严格 1:1，120/120 实测），出图复用 `EncounterPortraitSpriteProvider`，UI 走 qinglong `TierBadgeRenderer` 同款**世界空间** `SpriteRenderer`（非 UITK）挂 `ItemController.ShowCard`+`Cleanup` postfix；含数据漂移说明（活数据在运行期缓存而非 StreamingAssets）（**Draft v2，已过 Codex 对抗评审并修订池化重挂 / 层边界两项，待人工确认实施**）。
  - 配套 executor 指令：[`2026-06-08-package-merchant-portrait-executor-prompt.md`](2026-06-08-package-merchant-portrait-executor-prompt.md)。
- [`2026-06-08-package-merchant-card-art-replacement-plan.md`](2026-06-08-package-merchant-card-art-replacement-plan.md) — 同目标（让人看出包裹卖给哪个商人）的**替代手段**：**直接替换包裹卡插画**（用自制 1024² 图）而非叠角标。替换原点 `[HarmonyPostfix] ItemVisualsController.SetCardFrameMaterial`，在 per-card `materialInstance` 上 `SetTexture(_BaseMap)`；磁盘图经 `Texture2D.LoadImage` 加载（`<GameRoot>/BazaarPlusPlusV4/CustomCardArt/`），并内嵌 117 张文字占位图作为默认资源；池化靠重绑重触发（无 child/无 token，比 overlay 方案更轻）。文字版已在 in-run 卡与 CollectionPanel 预览验证通过；真实图后续按同名 `<templateId>.png` 覆盖（**IMPLEMENTED — 文字版管线已验证**）。
  - 文字版 executor 指令（先用文字占位图打通管线）：[`2026-06-08-package-merchant-card-art-replacement-text-executor-prompt.md`](2026-06-08-package-merchant-card-art-replacement-text-executor-prompt.md)。
- [`2026-06-10-collection-panel-filter-target-structure.md`](../../plans/collection-panel-filter-target-structure.md) — CollectionPanel merchant filter v4 目标结构：source `offerSegments`、普通池/稀有附魔池分段、47 个商人逐项筛选逻辑、22 个普通训练师保留，明确不做 Monster / Team Monster / Zurphin（**Draft，待确认实施**）。
- [`2026-06-10-collection-panel-lifesteal-derived-keyword-plan.md`](2026-06-10-collection-panel-lifesteal-derived-keyword-plan.md) — CollectionPanel Lifesteal 关键词补齐方案：只在 VM 投影层从 `TCardItem` tier attributes 派生 `EHiddenTag.Lifesteal`，修复 chip 可用性和关键词筛选，不扩大到 Ammo/Crit/Charge 等其他属性（**Draft，待确认实施**）。

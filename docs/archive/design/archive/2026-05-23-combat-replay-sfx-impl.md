...
- `Game/CombatReplay/CombatReplayRuntime.Warmup.cs`
- `Patches/Combat/ReplayStateAudioDiagnosticPatch.cs`
...
- [decompiled/TheBazaarRuntime/TheBazaar/GameServiceManager.cs](../../../../decompiled/TheBazaarRuntime/GameServiceManager.cs)
...
也就是说：原假设至少不够完整。还有别的“静音器”没有被列举/验证。继续按假设去“再多写一层修复”就是在重复 WarmReplayAudioBanksAsync 走过的弯路：方向感觉对、改完仍然没声音、反复 churn。
...
| `GameServiceManager.PauseOrUnpauseGame` 是边沿触发——`(false)` 在 `GamePaused == false` 时不触发 `GamePausedOrUnpaused`，等于 no-op | [GameServiceManager.cs:432-456](../../../../decompiled/TheBazaarRuntime/GameServiceManager.cs#L432) | 仅靠 `EnsureReplayAudioUnpaused` 早先版本只调 `PauseOrUnpauseGame(false)` 完全可能打水漂——这正是当前实现也直接补打 `PauseBusses(false)` 的原因 |
...
放在 `CombatReplayRuntime.Warmup.cs` 里，对外只有一个签名 `LogReplayAudioState(string label)`。内部以反射读：
...
放在 `Patches/Combat/ReplayStateAudioDiagnosticPatch.cs`。
...
- 修改：[Game/CombatReplay/CombatReplayRuntime.Warmup.cs` — 加 `LogReplayAudioState` (L970)、`EnsureReplayAudioReadyForPlayback` (L923)、`StopAllTrackedSfxEventInstances` (L1179)、`ReassertSfxVolumeFromPreferences` (L1239)
- 修改：`Game/CombatReplay/CombatReplayRuntime.Bootstrap.cs:139` — 调用点更名
- 注：ReplayStateAudioDiagnosticPatch(§4.3)最终未交付；诊断日志能力未保留，仅保留 EnsureAudioReadyForPlayback 的分层防御逻辑（AudioBankWarmer.cs）。
- 修改：[BazaarPlusPlus.csproj](../../../../src/BazaarPlusPlus/BazaarPlusPlus.csproj) — `FMODUnity` 加进 `<Reference>`

...
| 8 | **显式 continue sink 是唯一 agent 退出路径**；host tick/snapshot 发布后不得自动 `ReplayState.Exit()`。 | 当前 `BazaarAgentUiPlumbing.TryAdvanceReplay` 会在 `ReplayState && !IsReplaying` 时直接 `replay.Exit()`（`BazaarAgentUiPlumbing.cs:50-83`），会抢在外部脚本 `POST /v1/replay/continue` 前收尾，必须删除或永久关闭。 |
...
- 队列复刻 `BazaarAgentActionQueue`：`TaskCompletionSource`(`RunContinuationsAsynchronously`) + `Timer` 超时 + disposed→503（`BazaarAgentActionQueue.cs:21-101`）。`record` 的超时取较宽常量（主线程上 msgpack 解码 + `CombatReplayLoader.Load` 比一次决策重，但仍在一个 Tick 内完成 accept）。

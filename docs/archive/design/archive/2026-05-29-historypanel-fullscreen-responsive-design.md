...
- **不删除临时 F9 调参器 / 调试色块**（`HistoryPanel.PreviewTunerDebug.cs`），
  仅同步它依赖的缩放口径。
...
- [HistoryPanelUiToolkitView.cs:91-96](../../../../src/BazaarPlusPlus/Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.cs#L91-L96)：
...
- [HistoryPanelUiToolkitView.Tree.cs:13-34](../../../../src/BazaarPlusPlus/Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.Tree.cs#L13-L34)：
  全屏 `overlay`（半透明 `Colors.HistoryOverlay` 压暗 + `justifyContent/alignItems: Center`）里居中一个
  **固定 1280×1020**、圆角 `Radii.Panel` 的不透明 `panel`。
- 固定结构尺寸（[Sizes.cs:7-12](../../../../src/BazaarPlusPlus/Infrastructure/UiTokens/Sizes.cs#L7-L12)）：
...
- `BattleBoardPreview.cs:262-265`：独立
  `ScreenSpaceOverlay` Canvas（`sortingOrder = 27`），渲染真实卡片 prefab。
- [HistoryPanel.cs:362-385](../../../../src/BazaarPlusPlus/Game/HistoryPanel/HistoryPanel.cs#L362-L385) `ApplyPreviewContainerBounds`：
...
- 因此**容器一旦改成响应式，叠加层会自动跟随**——它本就读容器的真实屏幕矩形。唯一要对齐的是
  [HistoryPanel.cs:390-393](../../../../src/BazaarPlusPlus/Game/HistoryPanel/HistoryPanel.cs#L390-L393) `ComputePreviewPanelScale()`
  里写死的 `Screen.width / 1920`（假设了按宽缩放）。

### 开关 / 输入行为（全屏化需保持不回归）

- [HistoryPanelAccessPolicy.cs](../../../../src/BazaarPlusPlus/Game/HistoryPanel/HistoryPanelAccessPolicy.cs)：`CanOpen = !isInCombat`。
- [HistoryPanel.cs:160-188](../../../../src/BazaarPlusPlus/Game/HistoryPanel/HistoryPanel.cs#L160-L188) `Update()`：开战自动关闭；
...
设计 token（[Sizes.cs](../../../../src/BazaarPlusPlus/Infrastructure/UiTokens/Sizes.cs) 的字号/间距/行高）**保持不变**。
...
[HistoryPanelUiToolkitView.Tree.cs](../../../../src/BazaarPlusPlus/Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.Tree.cs)：
...
[Sizes.cs](../../../../src/BazaarPlusPlus/Infrastructure/UiTokens/Sizes.cs)：移除/改造结构性常量
...
- [Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.cs](../../../../src/BazaarPlusPlus/Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.cs)
  —— `PanelSettings.match`。
- [Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.Tree.cs](../../../../src/BazaarPlusPlus/Game/HistoryPanel/Ui/HistoryPanelUiToolkitView.Tree.cs)
  —— 去遮罩、面板铺满、直角、content/runs/preview 改相对。
- [Infrastructure/UiTokens/Sizes.cs](../../../../src/BazaarPlusPlus/Infrastructure/UiTokens/Sizes.cs)
  —— 移除/改造结构常量。
- [Game/HistoryPanel/HistoryPanel.cs](../../../../src/BazaarPlusPlus/Game/HistoryPanel/HistoryPanel.cs)
...
UI 布局改动、无自动化测试缝。按 .rules 可不强制 `BuildAll`：

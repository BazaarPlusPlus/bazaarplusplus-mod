...
- **Enchant preview** — appends post-enchant tooltip segments to hand / stash / opponent-board items. Driven by [ItemEnchantPreviewPatch.cs:54-57](../../../../src/BazaarPlusPlus/Patches/Tooltips/ItemEnchantPreviewPatch.cs#L54). Currently shown whenever `EnchantPreviewAlwaysShow` (bool, default `true`) is set **or** the `HoldEnchantPreview` hotkey (default Ctrl) is held. In-combat tooltips are excluded by [ItemEnchantPreviewEligibility.cs](../../../../src/BazaarPlusPlus/Game/ItemEnchantPreview/ItemEnchantPreviewEligibility.cs).
- **Upgrade preview** — flips the tooltip to the upgraded variant. Driven by [UpgradePreviewTooltipPatch.cs:35](../../../../src/BazaarPlusPlus/Patches/Tooltips/UpgradePreviewTooltipPatch.cs#L35). Currently shown **only** while the `HoldUpgradePreview` hotkey (default Shift) is held. There is no always-show flag.

Both flow through a single switching point: [TooltipModifierRefreshController.GetCurrentMode()](../../../../src/BazaarPlusPlus/Game/Tooltips/TooltipModifierRefreshController.cs), which returns `Upgrade | Enchant | Normal` once per frame and triggers a tooltip re-render when the mode changes.
...
- [`ChoiceState`](../../../../decompiled/TheBazaarRuntime/TheBazaar/ChoiceState.cs#L5) is the `RunAppState` while the player is on the map looking at offered encounters. `StateOps.SelectEncounter` is one of the allowed ops.
...
- [`Data.GetStatic().GetCardById(Guid)`](../../../../decompiled/TheBazaarRuntime/TheBazaar/Data.cs#L269) returns the `ITCard` template synchronously (the `Task` is already resolved once `IsManagerCreated()` is true).
...
The mod already has the abstraction for surfacing this data: [EncounterStateProbe](../../../../src/BazaarPlusPlus/GameInterop/Encounter/EncounterStateProbe.cs) returns an EncounterStateSnapshot every frame, holding things like `CurrentEncounterType` and `InteractionFilterTemplateIds`. Adding one more field to that snapshot is the natural extension point.
...
[TooltipModifierRefreshController.GetCurrentMode()](../../../../src/BazaarPlusPlus/Game/Tooltips/TooltipModifierRefreshController.cs#L47) returns the controller-private `TooltipModifierMode` enum (`Normal | Enchant | Upgrade`). The enum itself is unchanged. The body is replaced with an explicit, four-tier priority:
...
The existing eligibility filter inside [ItemEnchantPreviewService](../../../../src/BazaarPlusPlus/Game/ItemEnchantPreview/ItemEnchantPreviewService.cs) (in-combat exclusion, hand / stash / opponent-board scope) and the `card.CanCardUpgrade()` precondition in [UpgradePreviewTooltipPatch](../../../../src/BazaarPlusPlus/Patches/Tooltips/UpgradePreviewTooltipPatch.cs) both still apply. Auto-mode only flips the *mode* bit — it does not bypass per-item gating.

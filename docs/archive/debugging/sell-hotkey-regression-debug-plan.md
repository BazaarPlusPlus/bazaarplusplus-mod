...
> Status: PARTIAL/STALE: H3 resolved incidentally — NativeKeybindLabelPatch + _keybindAction reflection deleted at e8fc4d30 (2026-07-02); the diagnostics/native-conflict/tooltip-sell-safety steps never landed, and the 2026-05-31 native-path reads predate the current (PTR) game build. If the sell-hotkey regression resurfaces, start a fresh diagnosis instead of resuming this plan.

---
status: active
calibrated: 2026-06-10
note: "Investigation plan only. Native game-path claims must be rechecked against the current installed game/decompiled source before implementation."
---

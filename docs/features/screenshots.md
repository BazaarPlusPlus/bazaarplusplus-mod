...
3. 每条：读盘 PNG → 取 `player_account_id`（`BppClientCacheBridge.TryGetProfileAccountId`，取不到则**整轮短路跳过**，不发占位符）→ 组装 Snapshot DTO → `POST /bazaardb/snapshots/<snapshot_id>` → 按结果落库。
...
Ingest 把请求体原样写入私有 R2（`bazaardb/snapshots/<snapshot_id>/<upload_id>.json`）再写 D1 `bazaardb_delivery`；server 不解析 DTO、不解 base64、不校验 PNG。BazaarDB puller 通过 bearer-gated `peek` 拿短期预签 URL，成功落地后用 `confirm` 按 DTO 确认；confirm 后 R2 对象 best-effort 删除，D1 保留轻量 ledger 行。
...
- ingest 端点不鉴权，依赖自定义域访问边界；模组无上传 token。图片格式固定 PNG（Unity `EncodeToPNG`），但服务端只把 DTO 当 opaque JSON body 透传。

...
- The first graph probe reported `null`, but `_playableGraph` is declared on `BaseCardRevealAnimationDriver` while `CardEndOfRunAnimationDriver` inherits that base. The probe used a derived-type-only `GetField`, so `null` meant “field lookup failed or value null” and was not valid graph evidence. [`decompiled/TheBazaarRuntime/TheBazaar.Game.Cards.Animation/BaseCardRevealAnimationDriver.cs:18,91-92`; `decompiled/TheBazaarRuntime/TheBazaar.Game.Cards.Animation/CardEndOfRunAnimationDriver.cs:7`]
...
- The native raw-graph builder connects each `DelayPlayableBehavior` only to an `AnimationPlayableOutput`. Its completion check exists in `DelayPlayableBehavior.ProcessFrame`, where completed inputs cause `SetDone(true)`; Unity's [`ProcessFrame` contract](https://docs.unity3d.com/2021.3/Documentation/ScriptReference/Playables.PlayableBehaviour.ProcessFrame.html) requires the branch to be connected directly or indirectly to a `ScriptPlayableOutput`. [`decompiled/TheBazaarRuntime/TheBazaar.Game.Cards.Animation/BaseCardRevealAnimationDriver.cs:224-249`; `decompiled/TheBazaarRuntime/TheBazaar.Animation.Playables/DelayPlayableBehavior.cs:48-59`]
- The sibling non-raw graph builder creates exactly that missing `ScriptPlayableOutput` and connects it to output port 1. `Play` waits until every root is done before destroying the graph, while the end-of-run caller sets the cards' `FaceUp` animator parameter only after that await returns. Therefore the raw graph's missing process output blocks native finalization. [`decompiled/TheBazaarRuntime/TheBazaar.Game.Cards.Animation/BaseCardRevealAnimationDriver.cs:149-203,297-300`; `decompiled/TheBazaarRuntime/TheBazaar.UI.EndOfRun/EndOfRunSummaryController.cs:547-559`]

### Final ranked hypotheses

1. **Confirmed: raw graphs omit the process-pass output.** Adding the same port-1 `ScriptPlayableOutput` used by the non-raw builder should make each root become done after its clip input completes; native reveal completion and `FaceUp` should then allow capture and Continue without the fallback. [`decompiled/TheBazaarRuntime/TheBazaar.Game.Cards.Animation/BaseCardRevealAnimationDriver.cs:224-249,297-300`; `decompiled/TheBazaarRuntime/TheBazaar.Animation.Playables/DelayPlayableBehavior.cs:48-59`; `decompiled/TheBazaarRuntime/TheBazaar.UI.EndOfRun/EndOfRunSummaryController.cs:547-559`]
...
- The minimal seam is a Harmony postfix on `BaseCardRevealAnimationDriver.CreateRawGraph`, adding one `ScriptPlayableOutput` per root and sourcing output port 1. The decompiled runtime's two raw-mode call sites are the end-of-run reveal and disable sequences. [`decompiled/TheBazaarRuntime/TheBazaar.Game.Cards.Animation/BaseCardRevealAnimationDriver.cs:224-249,297-300`; `decompiled/TheBazaarRuntime/TheBazaar.UI.EndOfRun/EndOfRunSummaryController.cs:547-576`]

...
- The first graph probe reported `null`, but `_playableGraph` is declared on `BaseCardRevealAnimationDriver` while the runtime object is `CardEndOfRunAnimationDriver`. The probe used a derived-type-only `GetField`, so `null` meant “field lookup failed or value null” and was not valid graph evidence.
...
- The native raw-graph builder connects each `DelayPlayableBehavior` only to an `AnimationPlayableOutput`. Its completion check exists exclusively in `DelayPlayableBehavior.ProcessFrame`, but Unity documents that `ProcessFrame` is invoked only for playables connected directly or indirectly to a `ScriptPlayableOutput`.
- The sibling non-raw graph builder creates exactly that missing `ScriptPlayableOutput` and connects it to output port 1. Therefore the raw end-of-run graph never calls its own completion check, its roots never become done, `BaseCardRevealAnimationDriver.Play` never returns, and `EndOfRunSummaryController` never sets the cards' `FaceUp` animator parameter.

### Final ranked hypotheses

1. **Confirmed: raw graphs omit the process-pass output.** Adding the same port-1 `ScriptPlayableOutput` used by the non-raw builder should make each root become done after its clip input completes; native reveal completion, `FaceUp`, capture, and Continue should then occur without the fallback.
...
- The minimal seam is a Harmony postfix on `BaseCardRevealAnimationDriver.CreateRawGraph`, adding one `ScriptPlayableOutput` per root and sourcing output port 1. Search of the decompiled runtime shows raw mode is used only by the end-of-run reveal and disable sequences.

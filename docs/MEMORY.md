...
- The local SQLite schema is versioned; bump `RunLogSchema` (db=18, row=11, upload payload=6) when the persisted graph changes. [`src/BazaarPlusPlus.Storage/RunLog/RunLogSchema.cs:10-14`]
...
- CollectionPanel source filtering: embedded `collection-sources.json`, schema v4, `offerSegments` with typed per-enchantment rules; catalog locked to 71 sources / 49 merchants / 22 trainers. [`src/BazaarPlusPlus/Game/CollectionPanel/Sources/CollectionSourceCatalog.cs:15,259-351` | `tests/CollectionSourceFiltering.Tests/Program.cs:298-311`]
...
- Panel toggle hotkeys are filtered per registration by `HotkeyGuard` (returning false swallows the press); HistoryPanel deliberately won't close while a text field is focused — do not "fix" this. [`src/BazaarPlusPlus/Game/OverlayPanels/OverlayPanelHost.cs:72` | `Game/HistoryPanel/HistoryPanel.cs:458`]

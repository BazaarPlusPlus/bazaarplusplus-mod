...
- Runtime `Card.HiddenTags` does not carry static template hidden tags (`DTOUtils.CreateCard` never copies them and snapshot updates overwrite) — derive identity from `template.HiddenTags` and static data. [`decompiled/TheBazaarRuntime/TheBazaar/DTOUtils.cs`]

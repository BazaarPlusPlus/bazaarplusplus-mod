...
- The cloud backend lives in `bazaarplusplus-server` behind `mod-api-v4.bazaarplusplus.com`; verify server claims there, not from this repo.
...
- Game updates may add optional `AssetScope` parameters to `AssetLoader` methods. Cross-build calls share `NativeAssetLoaderInvocation`, and any asset retained in a process-static BPP cache must load in `Global`; `InRun`/`OutOfRun` handles are released on scene transitions and leave cached sprites or prefabs invalid. Collection compatibility smoke must cover native cards and every hero chip after a lobby↔run scope transition. [`src/BazaarPlusPlus/GameInterop/AssetLoading/`]

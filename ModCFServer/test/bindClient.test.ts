...
test("rebinding the same client and player account keeps a single active binding", async () => {
  const env = buildEnv();
  const { privateKey, modulusB64, exponentB64 } = generateClientKeyPair();
  const clientId = "runs-client-bind-idempotent";
  const installId = "install-bind-idempotent";
  env.DB.clients.set(clientId, {
    client_id: clientId,
    install_id: installId,
    purpose: "runs",
  ...
  for (const nonce of ["nonce-bind-idempotent-1", "nonce-bind-idempotent-2"]) {
    const bodyHash = sha256Base64(payload);
    const timestamp = new Date().toISOString();
    const signature = signCanonical(
      privateKey,
  ...
  const activeBindings = Array.from(env.DB.clientPlayerAccountBindings.values()).filter(
    (row) => row.client_id === clientId && row.unbound_at_utc == null,
  );
  assert.equal(activeBindings.length, 1);
  assert.equal(activeBindings[0]?.player_account_id, "player-account-001");
  assert.equal(env.DB.clientPlayerAccountBindings.size, 1);
});

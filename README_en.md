...
BazaarPlusPlus is a BepInEx mod for *The Bazaar*. It adds combat UI enhancements, monster and tooltip previews, run logging, an in-game history panel, local combat replay playback, and optional background upload features.
...
- Monster preview: right-clicking a monster or encounter card shows enemy items and skills, preferring `MonsterDatabase` and falling back to runtime encounter tracking when needed.
...
- Background upload: optional run and replay upload, performed only while the client is outside a live run.

...
收 mod 上传的 run bundle，把 artifact 落到 R2、把结构化投影写进 D1，并对外提供 ghost battle 查询和短时效的 replay 下载链接。所有路由现在都不鉴权——身份由 `?player_account_id=` query 参数携带（`/ghost-battles`），或由 mod 直接放进 request body（`/run-bundles`）。
...
| Binding | 类型 | 用途 |
| --- | --- | --- |
| `DB` | D1 database | runs / battles / run_bundles 投影、replay_tokens、seen_player_accounts |
...
| --- | --- | --- |
| `GET` | `/health` | 健康检查 |
| `POST` | `/run-bundles` | 上传 run bundle artifact 并投影到 D1（不鉴权；`player_account_id` 来自 body） |
| `GET` | `/ghost-battles?player_account_id=…` | 查询当前玩家作为 opponent 出现的 battle 列表（不鉴权；缺 query 参数 → 400） |
| `POST` | `/ghost-battles/:battleId/replay-link` | 申请 5 分钟有效的 replay 下载 URL（不鉴权） |
| `GET` | `/replays/:token` | 凭 replay token 流式下载 R2 artifact（不鉴权） |
...
| `0002_auth_simplification.sql` | 新增 `tokens`，移除 installation 链路（已被 0009 进一步清掉） |
...
| `0009_drop_auth_tables.sql` | 移除 `users` / `tokens`，鉴权概念整体退出 |
| `0010_drop_installation_id.sql` | 从 `runs` / `battles` / `run_bundles` 移除 `installation_id`，并重建 `run_bundles` 去掉历史 `UNIQUE(installation_id, run_id, payload_hash)` |
| `0011_create_seen_player_accounts.sql` | 新增 `seen_player_accounts` 注册表（替代 `users` 在 ghost-battles 对手过滤里的角色），并从历史 `run_bundles` 反向回填 |

## Known Limitations

以下行为是当前 rollout 阶段的有意取舍。在面向更广用户群之前需要重新评估。

- **Ghost-battles 是公开端点。** 任何持有目标 `player_account_id` 的调用方都能拿到这个账号作为对手出现过的 battle 列表。`player_account_id` 已经会随 `/run-bundles` 上传出现在响应里，所以"通过 token 防止扫描"的护栏在删登录之前就已经形同虚设；这只是把"一次性获取的 token"换成"一次性获取的 ID"。
...
- **`seen_player_accounts` 是自举式注册表。** 仅在 `/run-bundles` 处理时把上传者本人的 `player_account_id`（非 `anonymous-player`）upsert 进表。恶意上传者可以为自己注册一个伪 ID，但对手 ID 不会被自动写入——这与登录前的 `users` 表行为等价。
...
- D1 行**不会**自动删除。`run_bundles`、`runs`、`battles`、`replay_tokens`、`seen_player_accounts` 持续累加，需要外部 GC。
...
| --- | --- |
| `src/index.ts` | 路由表 |
| `src/features/v3/` | HTTP handler（uploadRunBundle / queryGhostBattles / createReplayLink / downloadReplay） |
| `src/crypto/` | sha256 / base64 / signature 帮助器 |

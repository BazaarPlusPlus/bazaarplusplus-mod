...
internal static class CurrentReplayRecordingText
{
    internal static string Tooltip(CurrentReplayRecordingSnapshot snapshot)
    {
        var text =
            snapshot.CanOpenReport
                ? T("View detailed battle report", "查看战斗详细记录", "查看戰鬥詳細記錄")
            : snapshot.CanReveal
                ? snapshot.Phase == CurrentReplayRecordingPhase.Degraded
                        ? T(
                            "Show recorded video (completed with warnings)",
                            "查看已录制视频（存在警告）",
                            "查看已錄製影片（存在警告）"
                        )
                    : snapshot.Phase == CurrentReplayRecordingPhase.Failed
                        ? T(
                            "Show previous recorded video",
                            "查看上一份已录制视频",
                            "查看上一份已錄製影片"
                        )
                    : T("Show recorded video", "查看已录制视频", "查看已錄製影片")
    ...
    internal static string RecordAgainTooltip(CurrentReplayRecordingSnapshot snapshot)
    {
        var text = T("Record this battle again", "重新录制这场战斗", "重新錄製這場戰鬥");
        return
            snapshot.Phase == CurrentReplayRecordingPhase.Failed

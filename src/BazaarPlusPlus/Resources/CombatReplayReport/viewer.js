(function () {
  ...
  const STATUS_ACTIONS = new Set(["Haste", "Slow", "Freeze"]);
  const ROUTINE_CARD_ACTIONS = new Set(["Cooldown", "Haste", "Slow", "Freeze"]);
  ...
  function translatedEntityType(copy, rawType) {
    switch (asString(rawType, "entity").toLowerCase()) {
  ...
  function renderLaneLabels(entities, copy, laneHeight) {
    ...
    for (let index = 0; index < entities.length; index += 1) {
      ...
      if (assetUrl) {
        const assetKind = entity.type.toLowerCase() === "item"
          ? "bpp-art-item"
          : entity.type.toLowerCase() === "skill"
            ? "bpp-art-skill"
            : "bpp-art-identity";
        const assetFrame = createElement(
          "span",
          "bpp-lane-art " + assetKind + " bpp-span-" + entity.span
        );
      ...
      label.append(
        createTextElement("small", "bpp-lane-type", translatedEntityType(copy, entity.type))
      );
      row.append(label);
      labels.append(row);
    }
  ...
  function isVisibleTimelineEvent(event, entityById) {
    if (isMetricOnlyEvent(event)) return false;
    if (event.kind.toLowerCase() === "card-attribute" && ROUTINE_CARD_ACTIONS.has(event.action)) {
      return false;
    }
    if (event.kind.toLowerCase() !== "effect-executed") return true;
    if (event.action === "CardModifyAttribute") return false;
    if (event.action === "CardHaste" || event.action === "CardSlow" || event.action === "CardFreeze") {
      return false;
    }
    if (!STATE_APPLY_ACTIONS.has(event.action)) return true;
    const source = entityById.get(event.sourceId);
    return event.action === "PlayerRageApply"
      && source
      && source.type.toLowerCase() === "skill";
  }

  function statusApplyAction(action) {
  ...
  function buildStatusRanges(model, entities, timelineWidth, laneHeight) {
    const entityIndex = new Map(entities.map((entity, index) => [entity.id, index]));
    const active = new Map();
    const ranges = [];

    function closeRange(range, endMs, endEvent) {
      range.endMs = Math.max(range.startMs + 1, Math.min(model.durationMs, endMs));
      range.endEvent = endEvent || range.lastEvent;
      ranges.push(range);
    }

    for (const event of model.events) {
      if (event.kind.toLowerCase() !== "card-attribute" || !STATUS_ACTIONS.has(event.action)) {
        continue;
      }
    ...
    for (const range of active.values()) {
      const remainingMs = Math.max(0, asFiniteNumber(range.lastEvent.currentValue, 0));
      closeRange(
        range,
    ...
    for (const range of ranges) {
      ...
      const relatedEvents = model.events.filter((event) =>
      ...
      range.x = 14 + Math.min(1, range.startMs / duration) * usableWidth;
      range.endX = 14 + Math.min(1, range.endMs / duration) * usableWidth;
      range.laneHeight = laneHeight;
      range.cluster = {
        lane: range.lane,
        x: range.x,
        y: AXIS_HEIGHT + range.lane * laneHeight + laneHeight / 2,
        role: "target",
        events: [range.startEvent],
        relatedEvents,
        token: range.action.toLowerCase(),
        icon: range.startEvent.icon,
        statusRange: range,
      };
    }
    return ranges.sort((left, right) => left.x - right.x || left.lane - right.lane);
  }
  ...
  function statusRangeColor(action) {
    if (action === "Haste") return [72, 223, 208];
    if (action === "Slow") return [242, 185, 102];
    return [142, 160, 255];
  }

  function drawStatusRanges(context, ranges, laneHeight) {
    for (const range of ranges) {
      const color = statusRangeColor(range.action);
      const y = AXIS_HEIGHT + range.lane * laneHeight + Math.max(5, laneHeight * 0.18);
      const height = Math.max(12, laneHeight * 0.64);
      const width = Math.max(2, range.endX - range.x);
      context.fillStyle = "rgba(" + color.join(",") + ",0.16)";
  ...
  function nearestCluster(canvas, clientX, clientY, hitIndex, statusRanges) {
    ...
    for (const range of statusRanges) {
      const top = AXIS_HEIGHT + range.lane * range.laneHeight;
      if (
        x >= range.x - 4
        && x <= range.endX + 4
        && y >= top
        && y <= top + range.laneHeight
      ) {
  ...
  function renderTimeline(root, model, copy, video) {
    ...
    const timelineEventCount = createTextElement(
      "span",
      "bpp-count-pill",
      formatNumber(timelineEvents.length) + " " + translate(copy, "event"),
      "timeline-event-count"
    );
    timelineMeta.append(timelineEventCount);
    ...
    function rebuildTimeline(preserveViewport) {
      ...
      const eventClusters = buildClusters(model, timelineEvents, entities, timelineWidth, laneHeight);
      statusRanges = buildStatusRanges(model, entities, timelineWidth, laneHeight);
      clusters = eventClusters.concat(statusRanges.map((range) => range.cluster))
        .sort((left, right) => left.x - right.x || left.lane - right.lane);
      hitIndex = createHitIndex(clusters);
      const visibleCount = timelineEvents.length + statusRanges.length;
      setText(timelineEventCount, formatNumber(visibleCount) + " " + translate(copy, "event"));
      canvas.setAttribute(
        "aria-label",
        translate(copy, "timelineTitle") + ", " + visibleCount + " " + translate(copy, "event")
      );
    ...
    for (const event of model.events) {
      if (event.kind.toLowerCase() === "health") {
        ...
        if (changedAttribute === "Health" && amount < 0) {
          ...
          const damageTypeKey = normalizedDamageType === "burn"
            ? "burn"
            : normalizedDamageType === "poison"
              ? "poison"
              : normalizedDamageType === "" || normalizedDamageType === "damage" || normalizedDamageType === "crit"
                ? "direct"
                : "other";
          result.damageTypes[targetSide][damageTypeKey] += damage;
  ...
  function renderDamageComposition(parent, statistics, copy) {
    const keys = ["direct", "burn", "poison", "other"];
    const labelKeys = {
      direct: "damageDirect",
      burn: "damageBurn",
      poison: "damagePoison",
    ...
    const activeKeys = keys.filter((key) =>
      statistics.damageTypes.player[key] + statistics.damageTypes.opponent[key] > 0
    );
    const card = createElement(
      "article",
      "bpp-stats-chart-card bpp-stats-composition-card",
      "statistics-damage-composition-card"
    );
    card.append(
      createTextElement(
        "h3",
        "bpp-stats-chart-title",
        translate(copy, "damageComposition"),
        "statistics-damage-composition-title"
      )
    );
    parent.append(card);
    ...
    if (activeKeys.length === 0) {
      card.append(
        createTextElement(
          "p",
          "bpp-inline-empty",
          translate(copy, "damageCompositionEmpty"),
          "statistics-damage-composition-empty"
        )
      );
      return function () {};
    }
    ...
    if (activeKeys.length === 1) {
      ...
      card.append(
        createTextElement(
          "p",
          "bpp-stats-chart-note",
          translate(copy, "damageCompositionSingle"),
          "statistics-damage-composition-note"
        )
      );
      const summary = createElement(
        "div",
        "bpp-composition-summary",
        "statistics-damage-composition-summary"
      );
      const typeLabel = translate(copy, labelKeys[key]);
      for (const side of ["player", "opponent"]) {
        const value = statistics.damageTypes[side][key];
        summary.append(
          renderFact(
            translate(copy, side) + " · " + typeLabel,
            formatCompactNumber(value) + (value > 0 ? " · 100%" : ""),
            "statistics-damage-composition-" + side
          )
        );
      }
      card.append(summary);
    ...
    const element = createElement(
      "div",
      "bpp-stats-chart bpp-damage-composition-chart",
      "statistics-damage-composition-chart"
    );
    element.setAttribute("role", "img");
    element.setAttribute("aria-label", translate(copy, "damageComposition"));
    card.append(element);
    const chart = createChart(element);
    if (!chart) return function () {};
    const totals = {
      player: keys.reduce((sum, key) => sum + statistics.damageTypes.player[key], 0),
      opponent: keys.reduce((sum, key) => sum + statistics.damageTypes.opponent[key], 0),
    };
    const colors = { direct: "#ff607d", burn: "#ff9a55", poison: "#9f7aea", other: "#8190a7" };
    chart.setOption({
      animation: false,
      backgroundColor: "transparent",
      color: activeKeys.map((key) => colors[key]),
      grid: { left: 74, right: 24, top: 52, bottom: 34 },
      legend: {
        top: 8,
        right: 16,
        textStyle: { color: "#aab7ca", fontSize: 11 },
        data: activeKeys.map((key) => translate(copy, labelKeys[key])),
      },
      tooltip: {
        trigger: "axis",
        confine: true,
        ...
        formatter: function (parameters) {
          ...
          for (const parameter of parameters) {
            rows.push(
              ...
              + ": <strong>"
              + formatCompactNumber(parameter.data.original)
              + "</strong> · "
              + parameter.data.value.toFixed(1)
              + "%"
            );
          }
          return rows.join("<br>");
      ...
      xAxis: {
        ...
        min: 0,
        max: 100,
        axisLabel: { color: "#8190a7", fontSize: 11, formatter: "{value}%" },
        splitLine: { lineStyle: { color: "rgba(111, 137, 170, 0.14)" } },
      },
      yAxis: {
        type: "category",
      ...
      series: activeKeys.map((key) => ({
        name: translate(copy, labelKeys[key]),
        type: "bar",
        stack: "damage-composition",
        barMaxWidth: 28,
        animation: false,
        data: ["player", "opponent"].map((side) => {
          const original = statistics.damageTypes[side][key];
          return {
            value: totals[side] > 0 ? original / totals[side] * 100 : 0,
            original,
          };
        }),
      })),
      aria: { enabled: true, label: { description: translate(copy, "damageComposition") } },
    });
    const resize = function () { chart.resize(); };
    window.addEventListener("resize", resize, { passive: true });
    return resize;
  ...
  function renderStatistics(root, model, copy) {
    ...
    const resizeComposition = renderDamageComposition(charts, statistics, copy);

...
cannot silently consume different Viewer builds.
...
install, Chromium/WebKit installation, typecheck, a clean frontend build, pure tests, and browser
behavior tests. The Release .NET build then generates its own Viewer copy under `obj/` from that
same locked source tree and embeds it, together with the ECharts license notices, in
`BazaarPlusPlus.dll`.

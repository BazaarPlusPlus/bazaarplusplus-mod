...
public sealed class CollectionEncounterGameTooltipTextTests
{
    ...
    public void Build_renders_ineligible_options_as_flat_dimmed_lines()
    {
        ...
        Assert.Contains(
            "<size=85%><line-height=1.15em><color=#8F8268>Add It to Your Bushel: (if you have a Bushel) Your Bushel gains 20 Heal</color></size>",
            text
        );
    ...
    public void Build_renders_outcome_groups_with_percentages()
    {
        ...
        Assert.Contains(
            "<size=85%><line-height=1.15em><color=#8F8268>· <indent=1em>Clear the Way: (if you have Powder Keg) Gain 5 Gold</indent></color></size>",
            text
        );
    }

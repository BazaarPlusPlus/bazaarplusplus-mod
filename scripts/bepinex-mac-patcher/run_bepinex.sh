...
# - Removing the signature clears hardened runtime restrictions that block DYLD injection.
# - Re-signing with allow-jit + allow-unsigned-executable-memory lets Harmony use mprotect W+X.
# - The process must have some signature on macOS 26 or the kernel will SIGKILL it.
app_path="${executable_path%/Contents/MacOS*}"
...
if [ -n "${is_apple_silicon}" ]; then
    ...
    # On arm64, Apple Silicon hardware enforces W^X at the CPU level, blocking all Harmony patches
    # regardless of code signing entitlements.
    # arch strips DYLD_INSERT_LIBRARIES from the environment, so pass it manually via -e.

...
BUNDLE_ROOT_NORMALIZER="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/normalize-macos-bundle-root.sh"
...
# Preserve duplicate game bundles outside the app permanently. Restoring them
# after codesign would invalidate the final state immediately.
if game_running; then
    echo "[BPP] Cannot repair macOS trampoline while The Bazaar is running. Close the game and retry." >&2
    exit 1
fi
bash "$BUNDLE_ROOT_NORMALIZER" "$GAME_ROOT"

...
- A deletion is proved by deleting: `./run.sh build --with-bazaaragent` and `./run.sh test` must both pass, because test projects compile fakes and source-shadow capsules that `build` never touches.
...
- When replacing a subsystem or migrating to a prototype, ship only the new version in place — done when the old implementation is deleted and both `./run.sh build --with-bazaaragent` and `./run.sh test` pass. A fallback path or a merged build chain running both is not a migration.
